...
namespace rrt_planner {
    ...
    double* RRTPlanner::sampleRandomPoint() {
        ...
        rand_point_[0] = random_double_x();  // random value within map width
        rand_point_[1] = random_double_y();  // random value within map height
    ...
    double* RRTPlanner::extendTree(const double* point_nearest, const double* point_rand) {
        ...
        candidate_point_[0] = point_nearest[0] + params_.step_size * (direction_x / norm);
        candidate_point_[1] = point_nearest[1] + params_.step_size * (direction_y / norm);
